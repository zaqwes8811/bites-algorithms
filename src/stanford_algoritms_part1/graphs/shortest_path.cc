...
int main() 
{
  ...
  try {
    ...
    int v_active = 1;
    track[v_active].d = 0;
    track[v_active].visited = true;
    
    // Work
    // похоже нужна очередь как в BFS
    while (track.empty()) {  // цикл не такой  
      const Neighbors& neighbors = graph[v_active];
      
      int root_d = track[v_active].d;
            
      // Поиск минимального элемента
      if (!v_.empty()) {
	vector<pair<int, int> >::iterator at = min_element(begin(v_), end(v_), 
	...
	track[v_active].visited = true;
      ...
      // Обходим соседей
      for_each_(Neighbor u, neighbors) {
	int v = u.end;
	int w = u.weight;
	if (!track[v].visited) {
