...
int main() 
{
  ...
  try {
	...
	foreach(Neighbor u, neighbors) {
	  int w = u.end;
	  if (!track[w].visited) {
	    int current_d = track[w].d;  // текущее расстояние
	    int new_d = root_d + u.weight;
	    if (current_d > new_d)
	      track[w].d = new_d; 
	...
	// Поиск минимальной
	if (!v.empty()) {
	  vector<pair<int, int> >::iterator at = min_element(begin(v), end(v), 
	    [](const pair<int, int>& o1, const pair<int, int>& o2) -> bool {
	      return o1.second < o2.second;
	    }
	  );
	  pair<int, int> min_val = *at;
